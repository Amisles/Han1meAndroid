 ...
 * 订阅内容页解析器。
 *
 * 页面结构（官网 /subscriptions 或 ?query=<作者名>）：
 * - 顶部两份内容相同的横向滚动已订阅作者条（移动端导航条 + 桌面端筛选条），作者卡片
 *   class 含 `subscriptions-artist-card artist-option`，当前选中的额外带
 *   `subscriptions-active-artist`（仅 ?query= 视图出现），按作者名去重（LinkedHashSet 保序）。
 *   头像取卡片内带 `position: absolute` 的那张 img（第一张是背景图），缺失时回退最后一张 img；
 *   名称取 `.card-mobile-title.search-artist-title`。
 * - 主体视频网格与首页/搜索一致，使用 `.video-item-container`，直接复用 [VideoListParser]。
 */
@Singleton
