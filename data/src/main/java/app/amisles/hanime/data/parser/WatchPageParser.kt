...
class WatchPageParser @Inject constructor(
    ...
    fun parse(html: String, baseUrl: String): VideoDetail? {
        return try {
            ...
            val main = parseMain(doc, html, baseUrl)
            val playlist = parsePlaylist(doc, baseUrl)
            val related = parseRelatedVideos(doc, baseUrl, playlist)
            main.copy(relatedVideos = related, playlist = playlist)
    ...
    fun parseStreaming(html: String, baseUrl: String): Flow<VideoDetailEvent> = flow {
        try {
            val doc: Document = Jsoup.parse(html, baseUrl)
            emit(VideoDetailEvent.MainInfo(parseMain(doc, html, baseUrl)))
            val playlist = parsePlaylist(doc, baseUrl)
            emit(VideoDetailEvent.Playlist(playlist))
            val related = parseRelatedVideos(doc, baseUrl, playlist)
            emit(VideoDetailEvent.RelatedVideos(related))
        } catch (e: IndexOutOfBoundsException) {
            emit(VideoDetailEvent.Error(e.message ?: "解析详情页失败"))
        } catch (e: NullPointerException) {
            emit(VideoDetailEvent.Error(e.message ?: "解析详情页失败"))
        } catch (e: IllegalArgumentException) {
            emit(VideoDetailEvent.Error(e.message ?: "解析详情页失败"))
    ...
    private fun parseMain(doc: Document, html: String, baseUrl: String): VideoDetail {
        ...
        val resolvedDefaultSourceUrl = if (defaultSourceUrl.isEmpty() && sources.isNotEmpty()) {
            (sources.find { it.size == 720 } ?: sources.first()).url
        } else {
            defaultSourceUrl
        }
        ...
        return VideoDetail(
            ...
            defaultSourceUrl = resolvedDefaultSourceUrl,
            tags = tags,
    ...
    private fun parsePlaylist(doc: Document, baseUrl: String): PlaylistInfo? {
        return playlistParser.parse(doc, baseUrl)
    }

    private fun parseRelatedVideos(doc: Document, baseUrl: String, playlist: PlaylistInfo?): List<HanimeVideo> {
